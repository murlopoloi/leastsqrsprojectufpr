#Done in Julia 1.1.1, 05/19/2020 (MM/DD/YYYY)
using Plots
...
function lsqm(vx,vy) #Linear least square
    m = size(vx,2)
    sumyi, sumxi, sumxiyi, sumxisq, sumerr = 0, 0, 0, 0, 0
    for i = 1:m
        sumyi = sumyi + vy[i]
        sumxi = sumxi + vx[i]
...
function expsqm(vx,vy) #Exponential least square
    ...
    for i = 1:m
        sumerr = sumerr + (ℯ ^ a₀ * ℯ ^ (a₁ * vx[i]) - vy[i]) ^ 2
    end
    return ℯ^a₀, a₁, sumerr
end
...
function quadsqm(vx,vy) #Quadratic least square
    m = size(vx,2)
    sumyi, sumxi, sumxiyi, sumxisq, sumxicb, sumxifth, sumxisqyi, sumerr = 0, 0, 0, 0, 0, 0, 0, 0
    for i = 1:m
        sumyi = sumyi + vy[i]
        sumxi = sumxi + vx[i]
        sumxiyi = sumxiyi + vx[i] * vy[i]
    ...
    a₁ = A - B * a₂
    for i = 1:m
        sumerr = sumerr + (a₀ + a₁ * vx[i] + a₂ * vx[i] ^ 2 - vy[i]) ^ 2
    end
    return a₀, a₁, a₂, sumerr
end

function sqexpsqm(vx,vy) #Exponential quadratic least square
    m = size(vx,2)
    sumyi, sumxi, sumxiyi, sumxisq, sumxicb, sumxifth, sumxisqyi, sumerr = 0, 0, 0, 0, 0, 0, 0, 0
    for i = 1:m
        sumyi = sumyi + log(vy[i])
    ...
    a₁ = A - B * a₂
    for i = 1:m
        sumerr = sumerr + (ℯ ^ (a₀ + a₁ * vx[i] + a₂ * vx[i] ^ 2) - vy[i]) ^ 2
    end
    return a₀, a₁, a₂, sumerr
end
